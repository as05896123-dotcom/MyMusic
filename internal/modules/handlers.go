...
Arabized & Modified for Slashless Commands */ package modules
...
type MsgHandlerDef struct { Pattern string Handler telegram.MessageHandler Filters []telegram.Filter }

type CbHandlerDef struct { Pattern string Handler telegram.CallbackHandler Filters []telegram.Filter }

// --------------------------------------------------------- //  تعديل الأنماط (Regex) لدعم العربي والإنجليزي بدون سلاش // --------------------------------------------------------- var handlers = []MsgHandlerDef{ {Pattern: "(?i)^/?(json|جيسون)$", Handler: jsonHandle}, { Pattern: "(?i)^/?(eval|قيم)$", Handler: evalHandle, Filters: []telegram.Filter{ownerFilter}, }, { Pattern: "(?i)^/?(ev|كود)$", Handler: evalCommandHandler, Filters: []telegram.Filter{ownerFilter}, }, { Pattern: "(?i)^/?(bash|sh|تيرمينال)$", Handler: shellHandle, Filters: []telegram.Filter{ownerFilter}, }, { Pattern: "(?i)^/?(restart|ريستارت|انعاش|إعادة تشغيل)$", Handler: handleRestart, Filters: []telegram.Filter{ownerFilter, ignoreChannelFilter}, },
...
var cbHandlers = []CbHandlerDef{ {Pattern: "start", Handler: startCB}, {Pattern: "help_cb", Handler: helpCB}, {Pattern: "^lang:[a-z]", Handler: langCallbackHandler}, {Pattern: ^help:(.+), Handler: helpCallbackHandler},
...
func Init(bot *telegram.Client, assistants *core.AssistantManager) { bot.UpdatesGetState() assistants.ForEach(func(a *core.Assistant) { a.Client.UpdatesGetState() })

// -----------------------------------------------------
//  FIXED: تسجيل handlers باستخدام AddHandler و NewMessage
// -----------------------------------------------------
for _, h := range handlers {
	// "message:" prefix optional depending on gogram version.
	// Here we keep the pattern as-is (regex that matches message text),
	// NewMessage will use the regex to match incoming messages.

	nm := &telegram.NewMessage{
		Pattern: h.Pattern,
		Handler: SafeMessageHandler(h.Handler),
		Filters: h.Filters,
		Group:   100,
	}
	bot.AddHandler(nm)
...
func ntgOnStreamEnd( chatID int64, _ ntgcalls.StreamType, _ ntgcalls.StreamDevice, ) { onStreamEndHandler(chatID) }

func setBotCommands(bot *telegram.Client) { // Set commands for normal users in private chats if _, err := bot.BotsSetBotCommands(&telegram.BotCommandScopeUsers{}, "", AllCommands.PrivateUserCommands); err != nil { gologging.Error("Failed to set PrivateUserCommands " + err.Error()) }
